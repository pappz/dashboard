/* eslint-disable */
import enabled from '../assets/enabled.gif'

var serverSupportsSSL = false;
var disableSSLMenu    = true;
var suppressAllAudio  = false;
var linkifyURLs       = 1;
var userCSSList       = [ ];
var offset
                  ...
                  if (entry.tagName == 'LI') {
                    if (++i >= begin) {
                      ...
                      if (i == current) {
                        ...
                        if (!sheet.disabled) {
                          label.innerHTML= `<img src="${enabled}" />` +
                                           label.innerHTML;
                        }
                      } else {
                        sheet.disabled   = true;
...
VT100.prototype.initializeElements = function(container) {
  ...
  if (container) {
    ...
    /*
     * Original line without external iframe js:
    ...
    '<table class="popup" ' +
      ...
      '<tr><td>' +
        '<ul id="menuentries">' +
          ...
          '<hr />' +
          '<li id="beginconfig">' +
             (this.utfEnabled ? `<img src="${enabled}" />` : '') +
             'Unicode</li>' +
          '<li>' +
             (this.visualBell ? `<img src="${enabled}" />` : '') +
             'Visual Bell</li>'+
          '<li id="endconfig">' +
             (this.blinkingCursor ? `<img src="${enabled}" />` : '') +
             'Blinking Cursor</li>'+
...
function ShellInABox(handler, url, container) {
  this.terminalDispatcher = handler
...
ShellInABox.prototype.sendRequest = function() {
  var content = {
        'width': this.terminalWidth,
        'height': this.terminalHeight
        }
  this.terminalDispatcher.openTerminal(content);
};

ShellInABox.prototype.onReadyStateChange = function(msg) {
    this.connected = true;
    this.vt100(msg);
};
...
ShellInABox.prototype.sendKeys = function(keys) {
  ...
  this.terminalDispatcher.sendKeys(content);
};
